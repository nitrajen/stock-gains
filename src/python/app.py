...
def prices_timeseries(symbol, start, end):
    ...
    df = get_historical_data(symbol, start, end, output_format='pandas',token='')
...
def prices_metrics(symbol, date):
    ...
    prices = get_historical_data(symbol, date, date, output_format='pandas',token='')
...
def returns_timeseries(symbol, start, end):
    ...
    df = get_historical_data(symbol, previous_day, end, output_format='pandas',token='')
